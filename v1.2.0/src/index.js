...
function starColor(stars){
    if (gamemode === 0){
        if (stars < 100) return '#AAAAAA';
        else if (stars < 200) return '#FFFFFF';
        else if (stars < 300) return '#FFAA00';
        else if (stars < 400) return '#55FFFF';
        else if (stars < 500) return '#00AA00';
        else if (stars < 600) return '#00AAAA';
        else if (stars < 700) return '#AA0000';
        else if (stars < 800) return '#FF55FF';
        else if (stars < 900) return '#5555FF';
        else if (stars < 1000) return '#AA00AA';
        else return '#FF5555';
    }
    else if (gamemode === 1){
...
function getTag(api){
    try{
        ...
        else if (api.id === 'df954981-d720-4b4d-84e1-9d294f703868') return ['DEV', '#AA00AA'];
        else if (api.id === '6440f5d5-cc30-428c-812d-eb892c5cd411') return ['QT♡', '#FFB69D'];
        else if (api.id === '2b034ebe-e151-4b75-a8a6-7c50d8c7fd29') return ['✨', '#E998B7'];
        else if (api.id === 'c2291b87-d894-461d-aca3-6be83fc51310' || api.id === '48ed8ffb-95ec-4647-b7c1-c5990d40a6f2' || api.id === '9b5aeb7e-3d9b-43b2-b026-b2e444da24ff' || api.id === '01f32cf7-8b1a-4d2f-8b15-b477c65f7fb7' || api.id === '01c59560-e601-4b9a-a84c-24877c485f63' || api.id === 'a3cef65d-ed74-4b73-9f8e-46db5d87d6a3' || api.id === '2f457183-cca4-4a3e-a923-a03af37de287') return ['QT', '#E998B7'];
...
function NWL(exp){
    ...
    return exp < 0 ? 1 : parseFloat(1 + reversePqPrefix + Math.sqrt(reverseConst + (2 / growth) * exp)).toFixed(0);
}
...
function updateHTML(){
    ...
    for (let i = 0; i < players.length; i++){
        ...
        if (players[i].api === undefined){
            ...
            if (players[i].avatar !== undefined){namehtml += players[i].namehtml; taghtml += players[i].taghtml; wshtml += players[i].wshtml; fkdrhtml += players[i].fkdrhtml; wlrhtml += players[i].wlrhtml; bblrhtml += players[i].bblrhtml; finalshtml += players[i].finalshtml; winshtml += players[i].winshtml;}
                ...
                namehtml += `<li style="color: ${starsColor}; background: url(${avatar}) no-repeat left center; background-size: 20px; padding-left: 25px;">${stars}${players[i].namehtml}</li>`; taghtml += ttaghtml; wshtml += twshtml; fkdrhtml += tfkdrhtml; wlrhtml += twlrhtml; bblrhtml += tbblrhtml; finalshtml += tfinalshtml; winshtml += twinshtml;
            }
        }
    }
    //con.log('changing 2');
    if (numplayers === 0){
        ...
        $('#playertitle').html(`${players.length} PLAYERS`);
...
function addPlayer(ign, e = 0){
    ...
    $.ajax({type: 'GET', async: true, url: playerdb+ign, success: (data) => {
        uuid = data.data.player.id; got = true; ign = data.data.player.username;
        if (got){
                    ...
                    if (data.player.displayname === ign){
                        ...
                        if (gamemode === 0){
                            ...
                            if (api.stats === undefined){}
                                ...
                                let tfkdr = 0, twlr = 0;//tbblr = 0;
                                if (api.achievements !== undefined && api.achievements.bedwars_level !== undefined){starsColor = starColor(api.achievements.bedwars_level); stars = `<span style="color: ${starsColor};">[${api.achievements.bedwars_level}✫]</span>`;}
                            ...
                            if (api.stats === undefined){}
                                ...
                                if (api.stats.SkyWars.skywars_experience !== undefined){
                                    ...
                                    if(xp >= 15000){
                                        ...
                                        for (var i = 0; i < xps.length; i++) {
                                            if(xp < xps[i]) {
                                                lvl = parseFloat(1 + i + (xp - xps[i-1]) / (xps[i] - xps[i-1])).toFixed(0);
                                                break;
                                            }
                                        }
                                    }
                                ...
                                if (api.networkExp !== undefined) tnwlhtml = `<li style="color: #F3FF00">${NWL(api.networkExp)}</li>`;
...
function gameStartNotif(){
    if (config.get('settings.notifs', true)){
        const notification = {
            ...
            body: 'Your bedwars game has started!',
        ...
        if (k !== -1){
            ...
            if (msg.indexOf('ONLINE:') !== -1 && msg.indexOf(',') !== -1){
                ...
                for (let i = 0; i < who.length; i++){
                    if (config.get('settings.unnick', false) && who[i] === usernick) who[i] = user;
                ...
                if (!contains){
                    if (config.get('settings.unnick', false) && sent){usernick = join; addPlayer(user); sent = false;}
                    else addPlayer(join);
                }
            }
            else if (msg.indexOf('has quit') !== -1){
                if (inlobby){players = [];} inlobby = false;
            ...
            else if (msg.indexOf('????????') !== -1 && msg.indexOf(':') === -1){if (config.get('settings.shrink', true)){currentWindow.setSize(currentWindow.webContents.getOwnerBrowserWindow().getBounds().width, Math.round(zoom*35), true); $('#show').css('transform', 'rotate(90deg)'); $('#titles').css('display', 'none'); $('#indexdiv').css('display', 'none');} gameStartNotif();}
...
$(() => {
    ...
    $('#discord').on('click', () => {shell.openExternal('https://discord.gg/eDnHTT3aBS');});
    $('#info').on('click', () => {
    ...
    $('#bw').on('click', () => {
        ...
        gamemode = 0; config.set('settings.gamemode', 0); players = []; updateArray();
    });
    $('#sw').on('click', () => {
        ...
        gamemode = 1; config.set('settings.gamemode', 1); players = []; updateArray();
    });
    $('#duels').on('click', () => {
        ...
        gamemode = 2; config.set('settings.gamemode', 2); players = []; updateArray();
    });
    ...
    $('#overall').on('click', () => {
        ...
        gmode = ''; config.set('settings.bwgmode', ''); players = []; updateArray();
    });
    $('#solos').on('click', () => {
        ...
        gmode = 'eight_one_'; config.set('settings.bwgmode', 'eight_one_'); players = []; updateArray();
    });
    $('#doubles').on('click', () => {
        ...
        gmode = 'eight_two_'; config.set('settings.bwgmode', 'eight_two_'); players = []; updateArray();
    });
    $('#threes').on('click', () => {
        ...
        gmode = 'four_three_'; config.set('settings.bwgmode', 'four_three_'); players = []; updateArray();
    });
    $('#fours').on('click', () => {
        ...
        gmode = 'four_four_'; config.set('settings.bwgmode', 'four_four_'); players = []; updateArray();
    });
